...
<div class="col-sm-4 mb-5">
  ...
  <?php 
 $sql = "SELECT request_id, request_info, request_desc, request_date FROM submitrequest_tb"; //req info
 ...
 if($result->num_rows > 0){
  while($row = $result->fetch_assoc()){
   ...
   echo 'Request ID : '. $row['request_id']; //REQ ID
   echo '</div>';
   echo '<div class="card-body">';
   echo '<h5 class="card-title">Request Info : ' . $row['request_info'] . '</h5>'; //req info
